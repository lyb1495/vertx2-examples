...
import com.github.iyboklee.ch5.persistor.MessageParser.MessageType;

public class TCPChatWithPersistorClientVerticle extends Verticle {
	...
	private void deployWorkerVerticle() {
		container.deployWorkerVerticle("TCPChatWithPersistorClientWorkerVerticle.java");
	}
					...
					if (message.getBoolean("exit", false)) {
						...
						if (sessionID != null) {
							...
							if ("/enter".startsWith(tokens[0]) && tokens.length == 2 && rmID == null) {
								message.putNumber("messageType", MessageType.ENTER.ordinal());
								message.putString("rmID", tokens[1]);
							}
							else if ("/leave".startsWith(tokens[0]) && tokens.length == 2 && rmID != null) {
								message.putNumber("messageType", MessageType.LEAVE.ordinal());
								message.putString("rmID", tokens[1]);
							}
							else if (rmID != null) {
								message.putString("rmID", rmID);
								if ("/whisper".startsWith(tokens[0]) && tokens.length >= 3) {
									message.putNumber("messageType", MessageType.WHISPER.ordinal());
									message.putString("whisperTarget", tokens[1]);
									message.getObject("body").putString("message", MessageParser.arrayToString(tokens, 2, tokens.length));
								}
								else {
									message.putNumber("messageType", MessageType.CHAT.ordinal());
								}
							}
							else {
