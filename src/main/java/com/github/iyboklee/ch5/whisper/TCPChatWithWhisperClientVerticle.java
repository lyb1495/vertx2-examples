...
public class TCPChatWithWhisperClientVerticle extends Verticle {
	...
	private void deployWorkerVerticle() {
		container.deployWorkerVerticle("TCPChatWithWhisperClientWorkerVerticle.java");
	}
						...
						public void handle(Buffer buffer) {
							if (payloadLength == -1) {
								...
								if (sessionID != null) {
									...
									if (message.getMessageType() == MessageParser.MessageType.WHISPER) {
										String from = message.getUsername();
										String msg = message.getBodymsg();
						...
						if (sessionID != null) {
							...
							if ("/enter".startsWith(tokens[0]) && tokens.length == 2 && rmID == null) {
								...
								if ("/whisper".startsWith(tokens[0]) && tokens.length >= 3) {
									message.putNumber("messageType", MessageParser.MessageType.WHISPER.ordinal());
